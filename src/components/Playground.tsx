            ...
            <ul className="space-y-2">
              {paginatedResults.map((row, i) => (
                <li key={startIndex + i} className="bg-white/5 rounded-lg p-3 text-slate-300">
                  {headers.length === 1 
                    ? (
                      <div className="break-words">{String(row[headers[0]])}</div>
                    )
                    : (
                      <div className="space-y-1.5">
                        {headers.map(h => (
                          <div key={h} className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-2 break-words">
                            <span className="text-cyan-400 font-medium text-xs sm:text-sm shrink-0 sm:w-32">{h}:</span>
                            <span className="text-slate-300 text-xs sm:text-sm flex-1 min-w-0">{String(row[h] ?? '')}</span>
                          </div>
                        ))}
                      </div>
                    )
